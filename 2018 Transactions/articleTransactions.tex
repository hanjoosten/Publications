...
\end{equation}
	...
	To describe the growth of a stateful history structure we can define stateful history structures recursively:
\begin{eqnarray}
...
\end{eqnarray}
	...
	one state is added to the storage for every event in $\Events$.
	However, if changes are on average much smaller than states, it can be more economical to store only the changes.
...
\label{req:shs2ihs}
\end{eqnarray}
	If we want to build an IHS event-by-event, we need an incremental definition.
	For this purpose we define an {\em incremental history structure} ($\id{IHS}$)
...
\label{req:increment}
\end{eqnarray}
	This mapping can be used as recursive step in the following (recursive) definition of predicate $\id{IHS}$, which characterizes an incremental history structure:
\begin{eqnarray}
