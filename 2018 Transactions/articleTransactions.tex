...
\end{equation}
	...
	If a DIS is initialized with an empty historic structure
	and it grows by means of $\id{happens}$, i.e. by one event at a time,
	then at any moment in time it consists of a finite number of events.
	Therefore, we can safely assume that set $\Events$ is finite in every practical situation.
...
\section{States}
	...
	A {\em stateful historic structure} (SHS) is a triple $\la\Events, \id{prec}, \id{state}\ra$ in which
	...
	Alternatively, we may write $(s,e)\in\id{state}$ because a function is a univalent and total relation
	...
	In a stateful historic structure, the occurrence of an event $e$ requires that $\id{state}(e)$ exists.
	So we define the occurrence of an event as a mapping, $\id{occurs}_{P,e,s}$, from one SHS to another:
\begin{equation}
\begin{array}{l}
...
\end{equation}
	...
	Let $\mathbb D$ represent the set of all conceivable state changes.
	Let $s_2\ominus s_1$ represent the change from state $s_1$ to state $s_2$.
	Let $\delta(e_1,e_2)$ represent the change $\id{state}(e_2)\ominus\id{state}(e_1)$.

	Each time a new event happens, a change of state takes place.
	This change is described in the following requirements:
\begin{eqnarray}
	&&(\exists e_2:(e_1,e_2)\in\id{prec})\ \vee\ \id{state}(e_1)=\mathcal I\label{eqn:totState}\\
	&&(e_1,e_2)\in\id{prec}\ \Rightarrow\ \id{state}(e_2)=\id{state}(e_1)\oplus\delta(e_1,e_2)\label{eqn:nextState}
\end{eqnarray}
	...
	Together, requirements~\ref{eqn:totState} and~\ref{eqn:nextState} define the function $\id{state}$ recursively.
\marginpar{claim}
	So for every finite SHS that has been constructed from the initial state by means of event occurrences,
	the function $\id{state}$ is defined.
	This ensures that every event in $\Events$ has precisely one state, which means that states are defined predictably in a DIS.
...
\section{Atomicity}
\label{sct:Atomicity}
	{\em Atomicity} means that multiple primitive actions are executed as one.
	So let us first define the notion of atomic action.
