...
\title{Ampersand uses Relation Algebra as a Programming Language}
...
\begin{abstract}
	...
	Beside being declarative, relation algebra comes with attractive prospects for developing big software.
...
\label{sct:Programming in RA}
	...
	for instance in replacing "\stmtText{The employee, [emp], is entitled to [increase]}" by
	...
	A \define{template} is a string in which substrings between square brackets serve as placeholders.
	In our example, the strings \stmtText{[emp]} and \stmtText{[increase]} are called placeholders.
	...
	In software, a statement is a template without placeholders.
...
\footnote{A phrase and a placeholder are linked by a pair from $\flip{\id{phrase}};\id{placeholder}$.}.
	...
	The sole purpose for having the relation \id{shownPhrase} is to detect a change
